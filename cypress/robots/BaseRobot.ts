...
export class BaseHands {
  ...
  getTextFromId(id : string){
    cy.get(`#${id}`).then(function(title)
             {
               let productTitle=title.text() 
                //cy.get('#ddmDeliveryMessage > b').invoke('text').as('invokeText')
                 cy.log(productTitle)
             })
             return this;
  }
  mouseOverWithId(id : string){
    cy.get(`#${id}`).trigger('mouseover')
    return this;
  }
  clickOnLocatorWithIndex(locatorName: string, locatorValue: string, index: number) {
    cy.get(`[${locatorName}="${locatorValue}"]`).eq(index);
    return this;
  }
